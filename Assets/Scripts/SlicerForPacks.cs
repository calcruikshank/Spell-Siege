...
public class SlicerForPacks : MonoBehaviour
{
    ...
    {
        if (!this.GetComponent<SlicerMover>().is_Touched)
        {
            return;
        }
    ...
    private void OnTriggerExit(Collider other)
    {
        ...
        Vector3 newNormal = transformedNormal.normalized * _forceAppliedToCut;
        rigidbody.AddForce(newNormal * 1.5f, ForceMode.Impulse);
        rigidbody.AddTorque(newNormal * 1.5f, ForceMode.Impulse);
        Rigidbody rigidbody2 = slices[0].GetComponent<Rigidbody>();
        Vector3 newNormal2 = -transformedNormal.normalized * _forceAppliedToCut;
        rigidbody2.AddTorque(newNormal2 * 1.5f, ForceMode.Impulse);
        rigidbody2.constraints = RigidbodyConstraints.FreezePositionZ;
    }
}
